...
def extract_w2v(w2vpath): 
    wav2vec_feature = np.load(w2vpath)
...
def extract_feat(l_utt):
    ...
    for line in l_utt:
        ...
        spec = np.expand_dims(spec, axis = 0).astype(np.float32)		# add 0 dim for torch
        # print(spec.shape)
...
_dir_dataset = '/data4/mahaoxin/data/ASVspoof/LA/train/flac'		# directory of Dataset
#_target_dir_base = '/data4/mahaoxin/data/WA/wa_test/deyiyintong/'
_dir_name = '/lfcc_final/'
_nb_proc = 24
